...
def memm_viterbi(sentence, pre_trained_weights, feature2id, beam_size=2):
    ...
    for k in range(2, n):
        ...
        for tag_u in possibleTags[k + 1]:
            for tag_v in possibleTags[k]:
                possible_histories = []
                for t in all_tag:

                    possible_histories.append((sentence[k], t, sentence[k - 1], tag_v, sentence[k - 2], tag_u, sentence[k + 1]))
                ...
                probabilities = [pi[(k - 2, possibleTags[k][i], tag_u)] * q[i] for i in range(len(possibleTags[k]))]
                index_best_proba = np.argmax(probabilities)
                bp[(k-1, tag_u, tag_v)] = possibleTags[k][index_best_proba]
                pi[(k-1, tag_u, tag_v)] = probabilities[index_best_proba]
